...
class AsyncDnsResolver(object):
    ...
    async def resolve_dns(self, domain, resolver):
        try:
            result = await resolver.query(domain, 'A')
            await asyncio.sleep(0.1)  # 控制并发数量
    ...
    async def main(self, ):
        ...
        tasks = [asyncio.create_task(self.resolve_dns(domain, resolver)) for domain in self.domain_dict]
